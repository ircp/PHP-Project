<?
	...
	try {
		...
		echo("</tr>");
		echo("<tr>");
		echo("<td class = 'text-left' >Card code: <font color = '#17a2b8'>".$cardcodeprefix."-".$cardcodesuffix."</font></td>");	
		echo("</tr>");
		echo("</tbody>");
		echo("</table>");
		echo("</div>");
	...
	//----------------
				...
				foreach ($RESULT[$SUMMARY_YEAR[$year]] as $x => $xresult) {
					...
					if ($_SESSION[session_id()]['MENU'][$menuid]['UPDATE'] == 1) {
						echo("<button type='button' class='btn  btn-outline-success'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('5', '".$menuid."', 'C', '".$key."|".$xresult['LEAVE_ID']."');\">แก้ไข</button>");						
					}
					if ($_SESSION[session_id()]['MENU'][$menuid]['DELETE'] == 1) {
						echo("<button type='button' class='btn  btn-outline-danger'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('5', '".$menuid."',  'D', '".$key."|".$xresult['LEAVE_ID']."');\">ลบ</button>");
					}			
					echo("</div>");
					echo("</td>");	
				...
				for ($x = 1; $x <= $sum_month; $x++) {
					...
					if (isset($SUMMARY_WORKING[$SUM_WORKING_YEAR[$year]][$x] )) {
						foreach ($SUMMARY_WORKING[$SUM_WORKING_YEAR[$year]][$x] as $m => $xresult) {
							...
							echo("<td  class='text-center'>");
							echo("<div class='btn-group' role='group' aria-label='Basic example'>");
							if ($_SESSION[session_id()]['MENU'][$menuid]['UPDATE'] == 1 && $xresult['WORKING_STATUS'] != 3 && $xresult['WORKING_STATUS'] != 5) {
								echo("<button type='button' class='btn  btn-outline-success'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('8', '".$menuid."', 'C', '".$key."|".$m."');\">อนุมัติ</button>");		
								echo("<button type='button' class='btn  btn-outline-danger'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('8', '".$menuid."', 'D', '".$key."|".$m."');\">บันทึกเป็นวันลา</button>");	
							}	
							else {
								echo("<button type='button' class='btn  btn-outline-success'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('8', '".$menuid."', 'C', '".$key."|".$m."');\" disabled>อนุมัติ</button>");	
								echo("<button type='button' class='btn  btn-outline-danger'data-toggle='modal' data-target='#myModal'  onclick = \"windowOpen('8', '".$menuid."', 'A', '".$key."|".$m."');\" disabled>บันทึกเป็นวันลา</button>");	
							}
							echo("</div>");
							echo("</td>");	
							echo("</tr>");						
