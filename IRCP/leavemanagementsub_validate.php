	...
	try {
		...
		if ($key == 3) {
			$sql = "	SELECT COUNT(*) COUNTN
						...
						WHERE ELD.LEAVE_DATE = ? AND USER.USER_ID = ?";
						
			$stmt = $conn->prepare($sql);
			$stmt->execute(array($parameter, $userid));
		...
		if ($key == 2) {
			...
			if ($syear == $eyear) {				
				if ($syear == $currentyear) {
					...
					$stmt = $conn->prepare($sql);
					$stmt->execute(array($explode[$countn], $explode[$countn] , $explode[$countn] ,$explode[$countn] , $userid));	
					...
					$stmt = $conn->prepare($sql);
					$stmt->execute(array($end, $end , $end ,$end , $userid));	
...
*/				
				...
				$stmt = $conn->prepare($sql);
				$stmt->execute(array($send, $send , $send ,$send , $userid));	
				...
				$stmt = $conn->prepare($sql);
				$stmt->execute(array($eend, $eend , $eend ,$eend , $userid));	
